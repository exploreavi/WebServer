...
public class HttpParser {
	...
	public int parseRequest() throws IOException {
		...
		if (cmd[0].equals("GET") || cmd[0].equals("HEAD") || cmd[0].equals("POST") || cmd[0].equals("PUT")
			...
			// parse query parametersPOST
			idx = cmd[1].indexOf('?');
			if (idx < 0)
				url = cmd[1];
			else {
				url = URLDecoder.decode(cmd[1].substring(0, idx), "ISO-8859-1"); // ascii
																					// set
		...
		if (cmd[0].equals("OPTIONS") || cmd[0].equals("TRACE") || cmd[0].equals("CONNECT")) {
			ret = 501; // not implemented
			return ret;
		}
		// Mandatory host header since http version 1.1
